    ...
    if (message.type === "OPEN_SIDEBAR") {
      ...
      chrome.tabs.query({
        ...
        if (tabs.length > 0 && tabs[0].id) {
          // 发送消息给内容脚本打开侧边栏
          chrome.tabs.sendMessage(tabs[0].id, { action: "OPEN_SIDEBAR" }, response => {
            sendResponse({
              success: true,
              message: "已发送打开侧边栏请求",
              tabInfo: tabs[0]
...
async function addUserToSelected(username: string): Promise<boolean> {
  try {
    ...
    // 检查用户是否已经在列表中
    const alreadyExists = state.selectedUsers.some(u => u.id === user.id || u.username === user.username)
...
async function handleSearch(query: string): Promise<TwitterSearchUser[]> {
  try {
    ...
    if (!foundResults && state.settings.twitterApiSettings.apiEnabled && 
 ...
 * 根据指定的排序方式对搜索结果排序
 */
function sortSearchResults(results: TwitterSearchUser[], sortBy: string = "relevance"): TwitterSearchUser[] {
  switch (sortBy) {
    case "followers":
...
function generateMockSearchResults(query: string): TwitterSearchUser[] {
  ...
  const celebrityUsers = [
    ...
    },
    {
      ...
      profile_image_url: "https://pbs.twimg.com/profile_images/1414439092373254147/JdS8yLGI_400x400.jpg",
      verified: true,
    ...
    },
    {
      ...
      profile_image_url: "https://pbs.twimg.com/profile_images/1483577865056702469/rWA-3_T7_400x400.jpg",
      verified: true,
  ...
  const matchedCelebrities = celebrityUsers.filter(user => 
...
async function getAppState(): Promise<AppState> {
  try {
    ...
    return {
      ...
      currentUsername: existingState.currentUsername || initialState.currentUsername,
...
async function analyzeUser(username: string): Promise<TwitterUserAnalysis> {
  try {
    ...
    // 检查缓存配置
    const cacheTimeMs = (state.settings.analysisSettings.analysisCacheTime || 24) * 60 * 60 * 1000
    ...
    // 检查AI设置
    if (state.settings.aiModelSettings.enabled && 
      ...
      try {
        ...
        try {
          ...
          // 获取用户推文
          userTweets = await twitterScraper.scrapeUserTweets(username, 20)
...
function generateMockAnalysis(username: string): TwitterUserAnalysis {
  ...
  const celebrityAnalyses: Record<string, TwitterUserAnalysis> = {
    "elonmusk": {
      ...
      analysisResult: {
        ...
        summary: "思维跳跃，对科技和未来充满热情，喜欢在社交媒体上分享想法和观点，经常引发讨论和辩论。",
        replyTemplates: [
    ...
    "BillGates": {
      ...
      analysisResult: {
        ...
        summary: "理性而富有同情心，关注全球性问题，擅长将复杂话题简化为可理解的信息，注重基于数据的决策。",
        replyTemplates: [
    ...
    "ylecun": {
      ...
      analysisResult: {
        ...
        interests: ["深度学习", "计算机视觉", "人工智能伦理", "科学研究", "技术教育"],
        communicationStyle: "技术性、精确、教育性",
        summary: "思维严谨，关注AI领域的技术进步和伦理问题，善于解释复杂概念，积极参与学术和公共讨论。",
        replyTemplates: [
  ...
  const possibleTraits = [
    "分析性思维", "创新", "冒险", "社交", "谨慎", "直言不讳", 
    "开放态度", "保守", "好奇心", "学术", "实用", "敏感",
    "积极乐观", "悲观", "幽默", "严肃", "节俭", "慷慨", 
    "自信", "谦虚", "独立", "合作", "有条理", "灵活"
  ]
  ...
  for (let i = 0; i < 5; i++) {
    ...
    interests.push(possibleInterests[Math.floor(Math.random() * possibleInterests.length)])
  }
  ...
  const replyTemplates = replyTemplateStructures.map(template => {
    return template
      .replace('[兴趣]', uniqueInterests[Math.floor(Math.random() * uniqueInterests.length)] || '当前话题')
      .replace('[相关话题]', uniqueInterests[Math.floor(Math.random() * uniqueInterests.length)] || '相关见解')
      .replace('[特质]', uniqueTraits[Math.floor(Math.random() * uniqueTraits.length)] || '分析性')
      .replace('[补充观点]', '这个主题的其他方面')
      .replace('[主题]', uniqueInterests[Math.floor(Math.random() * uniqueInterests.length)] || '当前讨论')
 ...
 * 生成回复
 */
async function generateReply(username: string, context: string): Promise<string> {
  try {
    ...
    if (state.settings.aiModelSettings.enabled && 
      ...
      try {
        ...
        try {
          ...
          recentTweets = await twitterScraper.scrapeUserTweets(username, 3)
          ...
          const reply = await aiService.generatePersonalizedReply(
            {
              ...
              communicationStyle: analysis.analysisResult.communicationStyle || "",
    ...
    if (analysis.analysisResult?.replyTemplates?.length) {
      let template = analysis.analysisResult.replyTemplates[
        Math.floor(Math.random() * analysis.analysisResult.replyTemplates.length)
      ]
      ...
      template = template
        ...
        .replace('[相关主题]', '相关话题')
        .replace('[兴趣领域]', analysis.analysisResult.interests[0] || '相关领域')
        .replace('[相关内容]', '相关观点')
        .replace('[补充观点]', '补充见解')
 ...
 * 批量分析多个用户
 */
async function batchAnalyzeUsers(usernames: string[]): Promise<Record<string, TwitterUserAnalysis>> {
  try {
