import { TwitterSearchUser, TwitterApiSearchResponse } from "../types/twitter";
...
class TwitterApiService {
   ...
   * 搜索Twitter用户
   */
  public async searchUsers(query: string, bearerToken?: string): Promise<TwitterSearchUser[]> {
    ...
    try {
      ...
      url.searchParams.append("user.fields", "description,profile_image_url,public_metrics,verified");
      ...
      if (!response.ok) {
        ...
        throw new Error(`Twitter API请求失败: ${response.status} ${response.statusText}`);
      }
   ...
   */
  private convertTwitterApiUsers(apiResponse: TwitterApiSearchResponse): TwitterSearchUser[] {
