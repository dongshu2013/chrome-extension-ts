...
class TwitterScraperService {
  scrapeUserTweets(username: string): string[] | PromiseLike<string[]> {
    throw new Error("Method not implemented.")
  }
  scrapeUserProfile(
    username: string
  ): TwitterSearchUser | PromiseLike<TwitterSearchUser> {
    throw new Error("Method not implemented.")
  }
  /**
   * 构建Twitter搜索URL
      ...
      userElements.forEach((element, index) => {
        try {
          ...
          const fullNameElement = element.querySelector(
            '[data-testid="User-Name"] a:first-child > div > div > span'
          )
          ...
          // 提取个人简介
          const bioElement = element.querySelector(
            '[data-testid="UserCell"] > div:nth-child(2)'
          )
          ...
          const statsElements = element.querySelectorAll(
            '[data-testid="UserCell"] > div:nth-child(3) span'
          )
  ...
  private parseCountText(text: string): number {
    try {
      ...
      if (
        ...
        cleanText.includes("k") ||
        cleanText.includes("千")
      ) {
        return num * 1000
      } else if (
        cleanText.includes("M") ||
        cleanText.includes("m") ||
        cleanText.includes("万")
      ) {
        return num * 10000
      } else {
        return num
